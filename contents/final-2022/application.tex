...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    {\color{white}{通过求解其特征方程以得到 $\sigma^{x}_{(z)}$ 的本征值:
    \begin{align*}
...
{\color{white}{    能量表象的本征矢满足 $H|n\rangle = E_{n}|n\rangle$, 则矩阵元为
    \begin{align*}
...
{\color{white}{根据 $J = 0$, 而 $-|J|\leq M\leq |J|$, 夹逼定理得到 $M= 0$. 而磁量子数守恒, 所以 $J_{1,z} + J_{2,z} = J_{z} = 0$. 已知 C-G 系数可以用于将 $|J,M;j_{1},j_{2}\rangle$ 以基矢 $|j_{1},m_{1};j_{2},m_{2}\rangle$ 展开, 代入上述讨论结果有
...
{\color{white}{由于是在 $(\vec{S}^{2},S^{z})$ 表象, 所以 $S^{z}$ 的矩阵一定是对角矩阵. 选定基矢为 $\{|s,m\rangle\}$, 即$|1,1\rangle=\begin{pmatrix}
      1\\
      0\\
      0
...
{\color{white}{分离变量法求解薛定谔方程 $\begin{aligned}
    ...
    {\color{white}{题目中定义的傅里叶变换是非幺正的, 代入结论的时候需要注意系数. 
...
\delta_{\vec{k}_{2}-\vec{q},\vec{k}_{1}}\delta_{\sigma^{\prime},\sigma}),\quad v(\vec{q}=0)\text{不发散}\\
  ...
  \begin{enumerate}
    ...
    {\color{white}{因为 $\hat{R}(2\pi) = \mathbb{I}$, 所以有 $|\theta - 2\pi\rangle = |\theta\rangle$. 对于算符 $\hat{N}$ 的本征态 $|N\rangle$ 有
  \begin{align*}    
