...
\begin{enumerate}
  ...
  {\color{gray}{  大量热化自旋表示充分随机, 所以 $\begin{aligned}
...
{\color{gray}{  因为密度矩阵的迹表示系统的总概率, 而概率必须归一化, 即 $\begin{aligned}
    \text{Tr}[\rho] = \sum_{i}p_{i} = \boxed{1}
  \end{aligned}$}}
  
  \item \textbf{如果 $\rho$ 是混态的密度矩阵, 那么 $\text{Tr}[\rho^{2}]$ 应满足?}
  
{\color{gray}{  对任意密度矩阵总有$\begin{aligned}
...
{\color{gray}{  推导极其麻烦, 建议直接背结论, 不要试图考场现推. 假设 $\begin{aligned}
...
{\color{gray}{  二维谐振子的哈密顿量用粒子数算符写作 $\begin{aligned}
...
\color{gray}{{  任意态均可通过基矢展开, 形式为 $\begin{aligned}
